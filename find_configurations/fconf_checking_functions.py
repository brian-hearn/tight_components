...
def count_repeated_pairs(family, threshold):
    # Count how many pairs of elements appear in at least 'threshold' different triples in the family
    pair_counter = Counter()
    for triple in family:
        for pair in combinations(triple, 2):
            pair = tuple(sorted(pair))
            pair_counter[pair] += 1
    return sum(1 for count in pair_counter.values() if count >= threshold)
